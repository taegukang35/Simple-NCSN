...
class NCSNTrainer():
    ...
    def __init__(self, ncsn, lr, dataloader, num_iters, beta1, beta2, checkpoints_folder, save_every):
        ...
        self.L = 10
        # self.sigmas = torch.Tensor([0.599**i for i in range(0, 10)]).to(self.device)
        self.sigmas = torch.Tensor([0.599**i for i in range(0, 10)]).to(self.device)
        self.ncsn = ncsn.to(self.device)
        self.ncsn_opt = Adam(
    ...
    def _loss_tensor(self, batch, which_sigmas):
        ...
        # which_sigmas = which_sigmas.to(self.device)
    ...
    def _save_model(self, curr_iter):
        ...
        save_path = self.save_folder + '/my_ncsn_' + str(save_num)
    ...
    def annealed_langevin_dynamics(self, trained_ncsn, eps, T, num_samples):
        ...
        x_prev = torch.rand(num_samples, *image_shape).to(self.device) #uniform
        # x_prev =  torch.load("tensor.pth").to(self.device)
        for i in range(L):
            ai = eps * self.sigmas[i]**2/self.sigmas[L-1]**2
            i_tensor = torch.Tensor([i]*num_samples).long().to(self.device)
            for t in range(T):
    ...
    def annealed_langevin_dynamics_perstep(self, trained_ncsn, eps, T):
        num_samples = 1
        L = len(self.sigmas)
        image_shape = self._get_image_shape()
        # x_prev =  torch.load("tensor.pth").to(self.device)
        x_prev = torch.rand(num_samples, *image_shape).to(self.device) #uniform
        #torch.save(x_prev, "tensor.pth")
        step_imgs = []
        step_imgs.append(x_prev[0, 0].detach().cpu().squeeze())
        with torch.no_grad():
            for i in range(L):
                ai = eps * self.sigmas[i]**2/self.sigmas[L-1]**2
                i_tensor = torch.Tensor([i]*num_samples).long().to(self.device)
                for t in range(T):
                    ...
                    x_prev = x_curr
                step_imgs.append(x_prev[0, 0].detach().cpu().squeeze())
        combined_image = torch.cat(step_imgs, dim=1) 
        save_image(combined_image, 'sampled_images' + '/steps.png', nrow=1)
        return x_curr
